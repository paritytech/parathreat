                ...
                paras: vec![
                    ...
                    (
                        ...
                        ParaGenesisArgs {
                            genesis_head: HeadData::default(),
                            validation_code: ValidationCode(
                                casino_runtime::WASM_BINARY.unwrap().to_vec(),
                            ),
                            para_kind: ParaKind::Parachain,
                        },
                    ),
...
pub mod casino {
    ...
    pub const ED: Balance = casino_runtime::EXISTENTIAL_DEPOSIT;

    pub fn genesis(para_id: u32) -> Storage {
        let genesis_config = casino_runtime::RuntimeGenesisConfig {
            system: casino_runtime::SystemConfig {
                code: casino_runtime::WASM_BINARY
                    .expect("WASM binary was not build, please build it!")
                    .to_vec(),
                ..Default::default()
            },
            balances: casino_runtime::BalancesConfig {
                balances: accounts::init_balances()
                    .iter()
                    .cloned()
                    .map(|k| (k, ED * 4096))
            ...
            parachain_info: casino_runtime::ParachainInfoConfig {
                parachain_id: para_id.into(),
                ..Default::default()
            },
            collator_selection: casino_runtime::CollatorSelectionConfig {
                invulnerables: collators::invulnerables()
                    .iter()
                    .cloned()
                    .map(|(acc, _)| acc)
            ...
            session: casino_runtime::SessionConfig {
                keys: collators::invulnerables()
                    .into_iter()
                    .map(|(acc, aura)| {
                        (
                            acc.clone(),                          // account id
                            acc,                                  // validator id
                            casino_runtime::SessionKeys { aura }, // session keys
                        )
                    })
                    .collect(),
            },
            polkadot_xcm: casino_runtime::PolkadotXcmConfig {
                safe_xcm_version: Some(SAFE_XCM_VERSION),
                ..Default::default()
            },
            sudo: casino_runtime::SudoConfig {
                key: Some(get_account_id_from_seed::<sr25519::Public>("Alice")),
            },
            ..Default::default()
        };
